-- Materiais, exames laboratoriais e substituições alimentares
...
CREATE TABLE public.student_materials (
  ...
  description text,
  link_url text,
  category text NOT NULL DEFAULT 'nutricao',
  created_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX idx_student_materials_aluno ON public.student_materials (aluno_id, created_at DESC);

CREATE TABLE public.student_lab_exams (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  personal_id uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  aluno_id uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
...
CREATE INDEX idx_student_lab_exams_aluno ON public.student_lab_exams (aluno_id, exam_date DESC NULLS LAST);

CREATE TABLE public.diet_substitutions (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_diet_substitutions_plan ON public.diet_substitutions (plan_id, position);

GRANT SELECT, INSERT, UPDATE, DELETE ON public.student_materials TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.student_lab_exams TO authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON public.diet_substitutions TO authenticated;
GRANT ALL ON public.student_materials, public.student_lab_exams, public.diet_substitutions TO service_role;

ALTER TABLE public.student_materials ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.student_lab_exams ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.diet_substitutions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Materiais aluno lê"
  ON public.student_materials FOR SELECT TO authenticated
  USING (aluno_id = auth.uid() OR personal_id = auth.uid());

CREATE POLICY "Materiais personal gerencia"
  ON public.student_materials FOR ALL TO authenticated
  USING (personal_id = auth.uid())
  WITH CHECK (personal_id = auth.uid());

CREATE POLICY "Exames aluno lê"
  ON public.student_lab_exams FOR SELECT TO authenticated
  USING (aluno_id = auth.uid() OR personal_id = auth.uid());

CREATE POLICY "Exames personal gerencia"
  ON public.student_lab_exams FOR ALL TO authenticated
  USING (personal_id = auth.uid())
  WITH CHECK (personal_id = auth.uid());

CREATE POLICY "Substituições via plano"
  ON public.diet_substitutions FOR ALL TO authenticated
  USING (
    EXISTS (
...
BEGIN
  ...
  LOOP
    ...
    SELECT new_meal_id, food, quantity, unit, kcal, protein_g, carbs_g, fat_g, notes, position
    FROM public.diet_meal_items WHERE meal_id = meal_row.id ORDER BY position;
  END LOOP;
