-- Anamnese e plano alimentar: permissões alinhadas ao profissional logado

DROP POLICY IF EXISTS "Personal gerencia anamnese dos alunos" ON public.anamnesis;
DROP POLICY IF EXISTS "Admin vê anamnese" ON public.anamnesis;
...
CREATE POLICY "Admin gerencia anamnese"
  ON public.anamnesis
  FOR ALL
  TO authenticated
  ...
  USING (
    ...
    OR (
      ...
      AND EXISTS (
        SELECT 1
        FROM public.profiles p
        WHERE p.id = diet_plans.aluno_id
          AND p.personal_id = auth.uid()
      )
    )
    OR public.has_role(auth.uid(), 'admin')
  ...
  IF NOT (
    ...
    OR EXISTS (
      ...
      WHERE p.id = _aluno_id
        AND p.personal_id = actor_id
    )
    OR (
      public.is_professional_user(actor_id)
      AND EXISTS (
        SELECT 1
        FROM public.student_invitations i
        WHERE i.accepted_by = _aluno_id
          AND i.personal_id = actor_id
